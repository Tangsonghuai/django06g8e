...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingyuanyin = this.ruleForm.shenqingyuanyin.replace(reg,'../../../django06g8e/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tuihuohuanhuo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tuihuohuanhuoCrossAddOrUpdateFlag = false;
						 this.parent.search();
