<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="工艺" prop="gongyi">
          <el-select :disabled="ro.gongyi" v-model="ruleForm.gongyi" placeholder="请选择工艺">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="工艺" prop="gongyi">
	      <el-input v-model="ruleForm.gongyi"
                placeholder="工艺" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../django06g8e/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huodongshangpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huodongshangpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
